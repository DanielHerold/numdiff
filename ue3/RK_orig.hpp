...
//  public:
    NewtonF(double tt, double hh, 
	    ...
	    : t(tt), yold(yyold), c(cc), A(AA), h(hh) {//cout<<"hello from NewtonF contruktor" <<endl;
};

    void Eval (const Vector<> & all_ki_old, const ODE_Function & func, Vector<> & all_ki_new) const
      {
//cout << "Hello from F.Eval" <<endl;
         int s=c.Size();
         int n=yold.Size();
         Vector<> sum(n);
	 Vector<> evalfunc(n);
...
if(((double)L2Norm(dF))<= tol){zero=0.0;cout << "Newton Abbr bei t="<<F.t<<endl; return;}
  CalcInverse(dF, dF);
  F.Eval(zero, func, evalF);

cout << "evalF" << evalF << endl;
...
public:
  ...
  virtual void Step (double t, double h, const ODE_Function & func,
                     const Vector<> & yold, Vector<> & ynew) const
  {
//cout << "Hello from implizitRK Step" <<endl;
    int n = yold.Size();
...
public:
  ImplicitMidPoint() : ImplicitRKMethod (1)
  {
    Matrix<> A(1,1);
    Vector<> b(1),c(1);
//cout <<"Hello from implicitMidPoint construktor" << endl;
    A=0.5;
    b=1;
    c=0.5;

    SetAbc (A,b,c);
//cout<<"Bye from implicitMidPoint construktor" <<endl;
  }
};
